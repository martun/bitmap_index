  ...
  for (auto encoding_type : encoding_types) {
    for (auto index_type : index_types) {
      ...
      boost::filesystem::create_directory(test_dir);
	    ...
	    std::shared_ptr<BitMap> bmp_result = index->lesser(range_end, flags).get();
...
TEST_F(BitmapIndexUnitTest, integer_value_lesser_query_test) {
...
TEST_F(BitmapIndexUnitTest, integer_value_lesser_query_corner_case_test) {
	// A vector of (Value, row_id) pairs.
	std::vector<std::pair<uint32_t, uint32_t>> values =
		{ { 15, 4 }, { 16, 5 }, { 17, 4 }, 
		  { 19, 5 }, { 25, 7 }, { 30, 4 }, { 40, 8 } };
...
TEST_F(BitmapIndexUnitTest, low_cardinality_lesser_test) {
	// A vector of (Value, row_id) pairs.
	std::vector<std::pair<uint32_t, uint32_t>> values =
		{ { 15, 4 }, { 16, 5 } };
...
TEST_F(BitmapIndexUnitTest, low_cardinality_lesser_test_2) {
	// A vector of (Value, row_id) pairs.
	std::vector<std::pair<uint32_t, uint32_t>> values =
		  { { 15, 4 }, { 16, 5 } };
  std::vector<uint32_t> expected_result = {};
...
TEST_F(BitmapIndexUnitTest, large_integers_test) {
  ...
  std::vector<uint32_t> expected_result = {7, 15, 17, 18, 19};
  run_lesser_test<uint64_t>(
...
TEST_F(BitmapIndexUnitTest, negative_large_integers_test) {
  ...
  std::vector<uint32_t> expected_result = {17, 18, 19, 25};
  run_lesser_test<int64_t>(
...
TEST_F(BitmapIndexUnitTest, negative_integer_value_range_query_test) {
  ...
  std::vector<uint32_t> expected_result2 = {25};
  run_lesser_test<int32_t>(
...
TEST_F(BitmapIndexUnitTest, large_integers_test_2) {
  ...
  std::vector<uint32_t> expected_result = {19};
  run_lesser_test<int64_t>(
